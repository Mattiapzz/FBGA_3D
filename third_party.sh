...
: '
This file is used to install the dependencies for the FBGA3D package.
'
...
if [ ! -d "libnpy" ]; then
  echo "${YELLOW}Downloading libnpy${NORMAL}"
  git clone --branch v1.0.1 --depth 1 https://github.com/llohse/libnpy.git
  echo "${YELLOW}libnpy is header-only, no need to compile${NORMAL}"
else
  echo "${YELLOW}libnpy already installed${NORMAL}"
fi
