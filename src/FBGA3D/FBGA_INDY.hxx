...
#ifndef FBGA_INDY_HXX
#define FBGA_INDY_HXX
...
private:
  ...
  real m_lat_tol = 0.0; //1e-6;
  real m_lat_tol_vmax = 0.0; //1e-6;
  ablation_flags m_ablation_flags; // Ablation flags
  real m_lateral_shrink_factor = 1.0;///1.0;
  //
...
public:
  ...
  void setup_std();
  FBGA_INDY();
  explicit FBGA_INDY(spline_data_collection const & spline_data);

  void setup_gggv(scaling_gggv_factors const & scaling_factors, engine_max_ggv const & engine_max_ggv);
  // main methods
...
public:
  ...
  void eval_constraint_satisfaction_check();

  [[nodiscard]] real eval_constraint_satisfaction(real s) const;

  [[nodiscard]] std::vector<real> eval_solution_constraint_satisfaction(trajectory_offset_and_angles_container const &TOA, solution_container const & sol) const;

  [[nodiscard]] solution_container get_solution() const;
