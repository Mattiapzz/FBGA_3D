...
FBGA_INDY::FBGA_INDY(spline_data_collection const & spline_data)
: gggv(spline_data)
{
  this->setup_std();
}
...
void 
FBGA_INDY::setup_gggv(scaling_gggv_factors const & scaling_factors, engine_max_ggv const & engine_max_data)
{
  this->gggv.set_scaling_factors(scaling_factors);
  this->gggv.set_engine_max(engine_max_data);
}
...
void 
FBGA_INDY::create_nodes_cells(trajectory_offset_and_angles_container const &TOA)
{
  this->Nodes.clear();
  this->Cells.clear();  
...
void 
FBGA_INDY::eval_Omega_xyz(GG::NodeStruct3D & node)
{
...
void 
FBGA_INDY::eval_Omega_prime_xyz(GG::NodeStruct3D & node)
{
  // Cache trigonometric values
  const real cos_mu  = std::cos(node.mu);
  const real sin_mu  = std::sin(node.mu);
...
void
FBGA_INDY::eval_g_xyz(GG::NodeStruct3D & node)
{
...
real FBGA_INDY::compute(trajectory_offset_and_angles_container const &TOA, const real v0, yellow_flag_data const & yellow_flag )
{
...
real
FBGA_INDY::max_lateral_performance_func(GG::NodeStruct3D const & node, real V)
{
  const real V_dot = this->eval_V_dot_Vatildex(node, V, 0.0);
...
real
FBGA_INDY::get_max_longitudinal_performance(
  ...
  const real V)
{
  ...
  const real a_tilde_x_top = this->eval_a_tilde_x_max(node, V, 0.0);
  ...
  if(!ok)
  {
    std::cout << "FBGA_INDY::get_max_longitudinal_performance: Error in solving the maximum longitudinal performance function for V = " 
...
real
FBGA_INDY::get_min_longitudinal_performance(
  const GG::NodeStruct3D & node,
  const real V)
{
  // create a lambda function only of V_dot
  ...
  {
    // std::cout << "FBGA_INDY::get_min_longitudinal_performance: Warning: V_dot_min = " 
  ...
  if(!ok)
  {
    std::cout << "FBGA_INDY::get_min_longitudinal_performance: Error in solving the minimum longitudinal performance function for V = " 
...
real
FBGA_INDY::min_longitudinal_performance_funcNOOPTI(
  const GG::NodeStruct3D & node,
  const real V,
  const real V_dot)
{
...
real
FBGA_INDY::min_longitudinal_performance_func(
  const GG::NodeStruct3D & node,
  const real V,
  const real V_dot)
{
...
real
FBGA_INDY::get_min_longitudinal_performance_YF(
  const GG::NodeStruct3D & node,
  const real V)
{
  // create a lambda function only of V_dot
  ...
  if(!ok)
  {
    std::cout << "FBGA_INDY::get_min_longitudinal_performance_YF: Error in solving the minimum longitudinal performance function for V = " 
              << V << ", a_tilde_x_bottom = " << a_tilde_x_bottom << "\n";
    std::cout << "\tF2solve(V_dot_0)   = " << F2solve(V_dot_0) << "\n";
    std::cout << "\tF2solve(V_dot_min) = " << F2solve(V_dot_min) << "\n";
    std::cout << "\tV_dot_sol          = " << V_dot_sol << "\n";
...
real
FBGA_INDY::min_longitudinal_performance_func_YF(
  const GG::NodeStruct3D & node,
  const real V,
  const real V_dot)
{
...
real
FBGA_INDY::max_longitudinal_performance_funcNOOPTI(
  const GG::NodeStruct3D & node,
  const real V,
  const real V_dot)
{
...
real
FBGA_INDY::max_longitudinal_performance_func(
  const GG::NodeStruct3D & node,
  const real V,
  const real V_dot)
{
...
FBGA_INDY::BY()
{
  ...
  for(integer ith=0; ith < static_cast<integer>(this->Cells.size()); ith++)
  {
    ...
    else
    {
      ...
      cell.V_1 = v_0;
      //
      // std::cout << "FBGA_INDY::BY: Found a NaN. meaning that current YF requested acceleration is not enough!\n";
      this->yellow_index = ith;
      ok_by = false;
      break;
    }
...
FBGA_INDY::BW()
{
  ...
  for (auto i = static_cast<integer>(this->Cells.size() - 1); i >= 0; i--)
  {
    ...
    else
    {
      ...
      {
        // std::cout << "FBGA_INDY::BW() >> No solution found for cell " << i << "\n";
        // set type
...
real 
FBGA_INDY::compute_total_time() const
{
  real T = 0;
...
real 
FBGA_INDY::compute_time(real s) const
{
  real T = 0;
  for(const auto & cell: this->Cells)
  {
...
inline void
FBGA_INDY::compute_context(const GG::NodeStruct3D& node, real V, real V_dot, Context & ctx) const
{
  // Basic trigonometric and kinematic calculations
    ...
    ctx.a_tilde_x_max_gg = node.alpha * this->gggv.a_x_max(V, ctx.a_tilde_z);
    ctx.a_tilde_x_min_gg = node.alpha * this->gggv.a_x_min(V, ctx.a_tilde_z);
    ctx.a_tilde_x_eng = this->gggv.a_x_eng(V);
    ctx.rho_max = this->gggv.rho_max(V, ctx.a_tilde_z);
    ctx.rho_min = this->gggv.rho_min(V, ctx.a_tilde_z);
    
    // Compute final bounds
    ctx.a_tilde_x_max = std::min(
        std::pow((1.0 - std::pow((std::abs(ctx.a_tilde_y_clip) / ctx.a_tilde_y_lim), ctx.rho_max)), (1.0 / ctx.rho_max)) * ctx.a_tilde_x_max_gg,
        ctx.a_tilde_x_eng
    );
    
    ctx.a_tilde_x_min = std::pow((1.0 - std::pow((std::abs(ctx.a_tilde_y_clip) / ctx.a_tilde_y_lim), ctx.rho_min)), (1.0 / ctx.rho_min)) * ctx.a_tilde_x_min_gg;
...
real 
FBGA_INDY::signed_distanceNOOPTI(GG::NodeStruct3D const & node, real V, real V_dot)
{
...
real 
FBGA_INDY::signed_distance(GG::NodeStruct3D const & node, real V, real V_dot)
{
  Context ctx;
  this->compute_context(node, V, V_dot, ctx);
...
real 
FBGA_INDY::signed_distance_YF(GG::NodeStruct3D const & node, real V, real V_dot)
{
  const real a_tilde_y = this->eval_a_tilde_y(node, V);
  const real a_tilde_lim_y = (this->eval_a_tilde_y_lim(node, V, V_dot) - this->m_lat_tol);
  const real a_tilde_x_max = this->eval_a_tilde_x_max(node, V, V_dot);
...
real 
FBGA_INDY::eval_V_next(GG::NodeStruct3D const & node, GG::CellStruct3D const & cell, real VDOT)
{
...
real 
FBGA_INDY::eval_V_prev(GG::NodeStruct3D const & node, GG::CellStruct3D const & cell, real VDOT)
{
...
real
FBGA_INDY::eval_a_tilde_x(GG::NodeStruct3D const & node, real V, real V_dot)
{
...
real
FBGA_INDY::eval_a_hat_x(GG::NodeStruct3D const & node, real V, real V_dot)
{
  const real cos_chi = std::cos(node.chi);
  const real sin_chi = std::sin(node.chi);
...
real
FBGA_INDY::eval_V_dot_Vatildex(GG::NodeStruct3D const & node, real V, real atildex)
{
...
real
FBGA_INDY::eval_V_dot_Vahatx(GG::NodeStruct3D const & node, real V, real ahatx)
{
  // omega_x = (sin(chi) * Omega_y + cos(chi) * Omega_x) * s_dot;
  // omega_y = (cos(chi) * Omega_y - sin(chi) * Omega_x) * s_dot;
  // omega_z = Omega_z * s_dot + chi_dot;
...
real 
FBGA_INDY::eval_a_tilde_y(GG::NodeStruct3D const & node, real V)
{
...
real 
FBGA_INDY::eval_a_tilde_z(GG::NodeStruct3D const & node, real V, real V_dot)
{
...
real
FBGA_INDY::eval_a_hat_y(GG::NodeStruct3D const & node, real V)
{
  // omega_x = (sin(chi) * Omega_y + cos(chi) * Omega_x) * s_dot;
  // omega_y = (cos(chi) * Omega_y - sin(chi) * Omega_x) * s_dot;
  // omega_z = Omega_z * s_dot + chi_dot;
...
real
FBGA_INDY::eval_a_hat_z(GG::NodeStruct3D const & node, real V, real V_dot)
{
  const real cos_chi = std::cos(node.chi);
  const real sin_chi = std::sin(node.chi);
  const real s_dot = V * cos_chi / (1 - node.n * node.Omega_z);
...
real 
FBGA_INDY::eval_a_tilde_y_lim(GG::NodeStruct3D const & node, real V, real V_dot)
{
...
real 
FBGA_INDY::eval_a_tilde_x_max(GG::NodeStruct3D const & node, real V, real V_dot)
{
  // TODO OPTIMIZE
  const real a_tilde_y = eval_a_tilde_y(node, V);
  const real a_tilde_z = eval_a_tilde_z(node, V, V_dot);
  const real a_tilde_y_lim = (node.alpha * this->gggv.a_y_lim(V,a_tilde_z) - this->m_lat_tol);
  const real a_tilde_y_clip = clip(a_tilde_y, -a_tilde_y_lim, a_tilde_y_lim);

  const real a_tilde_x_max = node.alpha * this->gggv.a_x_max(V, a_tilde_z);
  const real a_tilde_x_eng = this->gggv.a_x_eng(V); // no alpha scaling. Engine power do not depend on adherence

  const real rho_max     = this->gggv.rho_max(V,a_tilde_z);
  ...
  return std::min(
    std::pow((1-std::pow((std::abs(a_tilde_y_clip)/a_tilde_y_lim),rho_max)), (1/rho_max) ) * a_tilde_x_max, 
    a_tilde_x_eng
  );
}
...
real 
FBGA_INDY::eval_a_tilde_x_min(GG::NodeStruct3D const & node, real V, real V_dot)
{
  // TODO OPTIMIZE
  const real a_tilde_y = eval_a_tilde_y(node, V);
  const real a_tilde_z = eval_a_tilde_z(node, V, V_dot);
  const real a_tilde_y_lim = (node.alpha * this->gggv.a_y_lim(V,a_tilde_z) - this->m_lat_tol);
  const real a_tilde_y_clip = clip(a_tilde_y, -a_tilde_y_lim, a_tilde_y_lim);
  const real a_tilde_x_min = node.alpha * this->gggv.a_x_min(V, a_tilde_z);
  const real rho_min     = this->gggv.rho_min(V,a_tilde_z);
  ...
  return std::pow((1-std::pow((std::abs(a_tilde_y_clip)/a_tilde_y_lim),rho_min)), (1/rho_min) ) * a_tilde_x_min ;
}

// --------------------------------------------------------------------------------------------

real 
FBGA_INDY::eval_a_tilde_x_min_YF(GG::NodeStruct3D const & node, real V, real V_dot)
{
  // TODO OPTIMIZE
  const real a_tilde_y = eval_a_tilde_y(node, V);
  const real a_tilde_z = eval_a_tilde_z(node, V, V_dot);
  ...
  return std::max(
    std::pow((1-std::pow((std::abs(a_tilde_y_clip)/a_tilde_y_lim),rho_min)), (1/rho_min) ) * a_tilde_x_min ,
...
real
FBGA_INDY::eval_s_ddot(GG::NodeStruct3D const & node, real V, real V_dot)
{
...
integer FBGA_INDY::get_cell_idx(const real s) const
{
  ...
  if (s >= this->Cells[right].s_1) {
    const_cast<FBGA_INDY*>(this)->m_past_index = right; // Update past_index for next search
    return right;
  }
  // if s is less than the first cell's s_0, return -1 (not found)
  if (s < this->Cells[left].s_0) {
    const_cast<FBGA_INDY*>(this)->m_past_index = -1; // Update past_index for next search
    return -1;
  }
  ...
  while (left <= right) {
    ...
    if (s >= cell.s_0 && s < cell.s_1) {
      ...
      const_cast<FBGA_INDY*>(this)->m_past_index = mid;
      return mid;
    }
...
real 
FBGA_INDY::eval_Vmax(real s) const
{
...
real
FBGA_INDY::eval_Omega_x(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const real s_0 = this->Cells[cell_idx].s_0;
  const real s_1 = this->Cells[cell_idx].s_1;
...
real
FBGA_INDY::eval_Omega_y(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const real s_0 = this->Cells[cell_idx].s_0;
  const real s_1 = this->Cells[cell_idx].s_1;
...
real
FBGA_INDY::eval_Omega_z(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const real s_0 = this->Cells[cell_idx].s_0;
  const real s_1 = this->Cells[cell_idx].s_1;
...
real 
FBGA_INDY::eval_V(GG::NodeStruct3D const & node, GG::CellStruct3D const & cell,real s)
{
...
real 
FBGA_INDY::eval_V_dot(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const auto & cell = this->Cells[cell_idx];
...
real 
FBGA_INDY::eval_A_hat_x(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const auto & cell = this->Cells[cell_idx];
  const auto & node = this->Nodes[cell.ID0];
...
real
FBGA_INDY::eval_A_tilde_x(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const auto & cell = this->Cells[cell_idx];
  const auto & node = this->Nodes[cell.ID0];
...
real
FBGA_INDY::eval_A_tilde_y(real s)
{
  const integer cell_idx = this->get_cell_idx(s);
  const auto & cell = this->Cells[cell_idx];
  const auto & node = this->Nodes[cell.ID0];
...
real
FBGA_INDY::eval_A_tilde_z(real s)
{
  const integer cell_idx = this->get_cell_idx(s);
  const auto & cell = this->Cells[cell_idx];
  const auto & node = this->Nodes[cell.ID0];
...
real
FBGA_INDY::eval_g_x(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const auto & cell = this->Cells[cell_idx];
...
real
FBGA_INDY::eval_g_y(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const auto & cell = this->Cells[cell_idx];
  const auto & node_0 = this->Nodes[cell.ID0];
...
real
FBGA_INDY::eval_g_z(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
  const auto & cell = this->Cells[cell_idx];
  const auto & node_0 = this->Nodes[cell.ID0];
...
SegmentType 
FBGA_INDY::eval_segment_type(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
...
real 
FBGA_INDY::eval_alpha(real s) const
{
  const integer cell_idx = this->get_cell_idx(s);
...
FBGA_INDY::eval_shell_plot(GG::NodeStruct3D const & fictitious_node, const real Va, const real Vb, const real V_dot
) 
{
  ...
  for (size_type i = 0; i < num_points; ++i)
  {
    ...
    const real a_tilde_y_lim = fictitious_node.alpha *  this->gggv.a_y_lim(V,a_tilde_z);
    ...
    for(size_type j = 0; j < num_points; ++j)
    {
      ...
      const real a_tilde_x_max = std::min(
        std::pow((1-std::pow((std::abs(a_tilde_y_clip)/a_tilde_y_lim),rho_max)), (1/rho_max) ) * fictitious_node.alpha * this->gggv.a_x_max(V, a_tilde_z), 
        this->gggv.a_x_eng(V)
      );
      // compute a_tilde_x_min
      const real a_tilde_x_min = std::pow((1-std::pow((std::abs(a_tilde_y_clip)/a_tilde_y_lim),rho_min)), (1/rho_min) ) * fictitious_node.alpha * this->gggv.a_x_min(V, a_tilde_z);
      // store the point
...
output_plot_ggv_shell 
FBGA_INDY::eval_shell_plotpy(GG::input_plot_ggv_shell const & input_plot) 
{
  output_plot_ggv_shell output_plot;
  constexpr size_type num_points = NUMPTSPLOTSHELL;
  for (auto V : input_plot.v)
  {
    const real a_tilde_y_lim = input_plot.alpha *  this->gggv.a_y_lim(V,input_plot.az);
    const real delta_a_tilde_y = 2.0 * a_tilde_y_lim / static_cast<real>(num_points - 1);
    for(size_type j = 0; j < num_points; ++j)
    {
      const real a_tilde_y      = -a_tilde_y_lim + (static_cast<real>(j) * delta_a_tilde_y);
      const real a_tilde_y_clip = clip(a_tilde_y, -a_tilde_y_lim, a_tilde_y_lim);
      const real rho_max            = this->gggv.rho_max(V,input_plot.az);
      const real rho_min            = this->gggv.rho_min(V,input_plot.az);
      // compute a_tilde_x_max
      const real a_tilde_x_max = std::min(
        std::pow((1-std::pow((std::abs(a_tilde_y_clip)/a_tilde_y_lim),rho_max)), (1/rho_max) ) * input_plot.alpha * this->gggv.a_x_max(V, input_plot.az), 
        this->gggv.a_x_eng(V)
      );
      // compute a_tilde_x_min
      const real a_tilde_x_min = std::pow((1-std::pow((std::abs(a_tilde_y_clip)/a_tilde_y_lim),rho_min)), (1/rho_min) ) * input_plot.alpha * this->gggv.a_x_min(V, input_plot.az);
      // store the point
      output_plot.a_tilde_x_max.push_back(static_cast<float>(a_tilde_x_max));
      output_plot.a_tilde_x_min.push_back(static_cast<float>(a_tilde_x_min));
      output_plot.a_tilde_y.push_back(static_cast<float>(a_tilde_y));
...
void 
FBGA_INDY::eval_constraint_satisfaction_check()
{
...
real 
FBGA_INDY::eval_constraint_satisfaction(real s) const
{
  // This function is used to evaluate the constraint satisfaction for the FBGA_INDY method.
  // It computes the necessary parameters and updates the context accordingly.
  Context ctx0;
  ...
  real tmp4 = 0.0;
  //
  // find cell containing s
  const integer cell_idx = this->get_cell_idx(s);
  if (cell_idx == -1) {
    std::cerr << "Error: s = " << s << " is out of bounds." << std::endl;
...
std::vector<real> 
FBGA_INDY::eval_solution_constraint_satisfaction(
  trajectory_offset_and_angles_container const &TOA, solution_container const & sol) const
{
  std::vector<real> cs_out;
  std::vector<NodeStruct3D> nodes_cs;
  const size_type N = TOA.reference.abscissa.size();
  nodes_cs.resize(N);
  cs_out.resize(N); // there are N-1 cells for N nodes
  ...
  {
    const auto& node0 = nodes_cs[i];
    const auto& node1 = nodes_cs[i+1];
    const real V0    = sol.V0[i]; // V_0 is already computed in the FW method
    const real V1    = sol.V1[i]; // V_1 is already computed in
    const real V_dot = sol.V_dot[i]; // V_dot is already computed in the FW method
    ...
    cs_out[i] = std::max(cs0, cs1);
  }
  // last cell
  cs_out[N-1] = cs_out[N-2];
  return cs_out;
...
solution_container 
FBGA_INDY::get_solution() const
{
  solution_container sol;
  ...
  for(size_type i = 0; i < N; i++)
  {
    sol.V0[i]    = this->Cells[i].V_0; // V_0 is already computed in the FW method
    sol.V1[i]    = this->Cells[i].V_1; // V_1 is already computed in
    sol.V_dot[i] = this->Cells[i].V_dot; // V_dot is already computed in the FW method
  }
  return sol;
}
