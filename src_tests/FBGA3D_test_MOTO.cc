...
int main(int argc, char *argv[])
{
  ...
  std::cout << "FBGA MOTO test started\n";
  std::cout << "Example class FBGA_MOTO\n";
  ...
  GG::real T = fbga_moto.compute(TOA, v_initial, yellow_flag_data);
  ...
  if(output_write)
  {
    ...
    std::string output_file_path = folder_path + "FBGA_MOTO" + base_circuit_name + "_output_" + note + ".csv";
    ...
    if (output_file.is_open()) {
      ...
      for (GG::integer i = 0; i < numpts_gg; ++i) {
        ...
        << fbga_moto.eval_A_hat_x(S_gg[i]) << ","
        << fbga_moto.eval_A_hat_y(S_gg[i]) << ","
        << fbga_moto.eval_A_hat_z(S_gg[i]) << ","
        << fbga_moto.eval_A_tilde_x(S_gg[i]) << ","
